...
4. 如 `setting/chapter-groups/` 下存在当前章节组细纲，优先读取对应细纲，确认本章在组内的作用、冲突、信息揭示和钩子
5. 必须读取前面至少 2 章定稿正文，确保本章与前文自然衔接
6. 根据大纲方向、章节组细纲、角色状态和前文正文，创作本章内容
7. 草稿流程关闭时，使用 write_file 将章节写入 `chapters/chXX-章节名.md`；草稿流程启用或用户明确要求草稿时，先写入 `drafts/chXX-章节名.md`
8. 不更改 `setting/outline.md`
9. 只有写入 `chapters/` 或作者明确确认定稿后，才使用 write_file/update_file 更新 `setting/progress.md`（当前进度 + 本章摘要）
10. 只有章节定稿后，才使用 read_file + write_file/update_file 更新 `setting/characters.md`（角色状态变化）
...
- 未定稿草稿不写入 `progress.md` 或 `characters.md`
