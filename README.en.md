...
- **Lore and presets**: maintain durable settings such as characters, worlds, locations, factions, rules, and items; narrative styles handle prose, prompt slots, and scene style rules, Story Directors handle event packs, stat/TRPG systems, opening traits, and pacing strategies, and image presets handle visual style.
...
Game Mode includes built-in Story Director orchestration. The Interactive Agent interprets player actions and produces the turn-level TurnBrief, while backend tools only perform deterministic work such as schema validation, numeric settlement, dice rolls, and expression-based rule checks. A background Director Agent reads the Story Director, lore, and interactive memories to maintain the mainline plan, event queue, foreshadowing, long-term arcs, and failure states. Creators can configure event card packs for face-slapping payoffs, hidden-strength reveals, encounters, secret realms, academy competitions, romance, misunderstandings, and reconciliation; each card carries a type name, Markdown event description, trigger scene, payoff/recovery notes, and scheduling metadata. The Config Manager Agent can inspect the lore index, read a bounded set of relevant entries, and generate world-grounded event card packs. Opening trait rolls, stat attributes, and reusable rule-check templates are also configurable.
