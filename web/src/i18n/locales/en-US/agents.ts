const agents = {
  ...
  'agents.context.recentTurnsNote': 'Model profile context window tokens default to 400K. When context usage reaches the threshold, Nova synchronously compacts older history and keeps the compaction summary plus recent raw turns in model context. Recent turns are only the fallback window when no model context window is configured or compaction is disabled.',
