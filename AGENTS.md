...
- 测试或验证如果依赖后端变更，优先使用用户已经启动的可见后端，或明确告知用户需要重启；确需由 Codex 重启后端时，必须使用用户可见的前台命令，不允许使用 launchd、nohup、后台守护、自动重拉等不可见方式启动或替换 Denova 前后端进程，也不允许在用户已有项目前后端时私自启动另一套前后端。
