...
- read_file：读取文件内容；结果首行包含完整文件的 sha256 revision，即使使用 offset/limit 只读取部分内容，该 revision 仍对应完整文件
...
- write_file：创建或覆盖整个文件（适合新建文件或全量重写）；覆盖已有文件必须传最近一次 read_file 返回的 base_revision，明确新建文件时传 base_revision="missing"
- edit_file：在单个文件中批量执行精确替换（参数：file_path, base_revision, edits）；base_revision 必须来自最近一次 read_file，版本冲突后先重新读取再重试
