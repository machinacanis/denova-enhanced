...
type SystemInstructionInput struct {
	...
	// StyleRules 是当前导演的场景化风格规则；调用方需先按本轮 # 选择和大小上限过滤。
...
5. 风格参考由导演场景化风格规则提供；本轮 # 只用于选择当前叙事编排中的场景风格，不代表文件引用
6. 仅当 system prompt 注入了场景化风格规则且任务属于章节正文创作/续写/重写/互动故事正文生成时，才参考这些已注入的风格内容
...
- %s/.nova/lore/ — 结构化资料库内部存储，承载角色、世界观、地点、势力、规则、物品等长期设定；优先通过 WebUI 资料库或配置管理 Agent 维护
