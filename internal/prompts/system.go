...
- read_file：读取文件内容
- list_lore_items：分页浏览或按多个 keywords、match、types 筛选资料库轻量索引；只返回目录，确认相关条目后再用 read_lore_items 读取正文
...
5. 资料库采用渐进式加载：常驻资料库正文已在当前作品状态中直接提供；按需资料先用 list_lore_items 浏览或筛选轻量索引，再调用 read_lore_items 读取少量相关正文
