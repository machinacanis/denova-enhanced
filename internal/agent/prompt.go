...
func configManagerFlowInstructionFor(workspace, creator string) string {
	...
	sb.WriteString(strings.Join([]string{
		...
		"- 叙事风格只维护文风、提示词槽位、场景风格和上下文策略；故事导演维护编排策略，并通过 module_refs 可插拔组合叙事风格、多个事件包、TRPG 检定、状态系统、开局选择器和图像方案；事件包只维护事件卡列表；每个 TRPG 检定资源代表一种 DM 检定风格，只维护一条 d20/d100 骰子配置（骰子类型、难度修正、失败处理、触发标准、必须检定/不要检定例子、难度判断、状态影响指引和后果写法）；状态系统是结构化状态和可计算字段的唯一真源；图像方案只维护视觉风格、媒介、构图、限制和避免项。",
