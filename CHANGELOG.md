...
- 游戏模式：新增 `POST /api/interactive/actor-traits/roll` 用于预览或固定 Actor 词条；`apply_actor_state_patch.trait_changes` 支持 `draw`、`reroll`、`set`、`remove`，正文 Agent 只接收来源明确、按可见性过滤且不超过 64KB 的当前 Actor 状态摘要。
- Game Mode: Added `POST /api/interactive/actor-traits/roll` for Actor trait previews and fixed selections. `apply_actor_state_patch.trait_changes` supports `draw`, `reroll`, `set`, and `remove`, while the prose Agent receives only a sourced, visibility-filtered current Actor state summary capped at 64KB.
...
- 方案预设：目录新增按名称、描述和标签搜索并保留用户展开状态；新建方案的名称、说明、标签和默认内容现在跟随当前中英文界面，拖拽等控件补齐本地化无障碍名称。
- Presets: Added directory search across names, descriptions, and tags while preserving user expansion state. Newly created preset names, descriptions, tags, and default content now follow the active Chinese or English locale, with localized accessible names for drag controls and related actions.
