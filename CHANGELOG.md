...
- 游戏模式：新增叙事编排基础能力，互动 Agent 可提交 TurnBrief，并通过 `prepare_interactive_turn` 执行固定数值、骰子、安全表达式、资源和终局候选检定；回合会持久化 RuleResolution 审计数据，自动追加 `director_patch` 更新后台 DirectorState。新增独立 `interactive_director` 后台导演 Agent，回合落盘后异步更新计划并记录 ready/failed 状态；前端支持导演状态读取/更新/重建、强制/禁用事件、规则重抽、开局词条抽取，以及在方案预设中配置主线强度、失败策略、节奏曲线、事件包和自定义事件。终局分支默认禁止继续追加普通回合，需要从历史节点创建新分支。
- Game Mode: Added the narrative orchestration foundation. The interactive agent can submit a TurnBrief and use `prepare_interactive_turn` for fixed numeric, dice, safe-expression, resource, and terminal-candidate checks; turns now persist RuleResolution audit data and append automatic `director_patch` updates to DirectorState. Added an independent `interactive_director` background Director Agent that asynchronously updates the plan after turn persistence and records ready/failed status; the frontend now supports Director read/update/rebuild, force/disable event actions, rule rerolls, opening-trait rolls, and teller-preset configuration for mainline strength, failure policy, pacing curve, event packages, and custom events. Terminal branches now block normal continuation by default and require branching from history.
...
- 游戏模式：新增独立“故事导演 / Story Director”资源、`/api/story-directors` CRUD、配置管理 Agent 工具和设置页编辑器；故事导演集中管理导演策略、事件系统、数值系统、TRPG 检定和开局选择器。
- Game Mode: Added standalone Story Director resources, `/api/story-directors` CRUD, Config Manager Agent tools, and a settings editor. Story Directors now own strategy, event systems, stat systems, TRPG checks, and opening selectors.
- 游戏模式：新增可独立配置的事件系统、数值&TRPG系统、开局选择器模块资源，提供 `/api/event-systems`、`/api/rule-systems`、`/api/opening-selectors` CRUD，并在方案预设页形成“模块库 + 故事导演组合器”。
- Game Mode: Added independently configurable Event System, Rule/TRPG System, and Opening Selector module resources with `/api/event-systems`, `/api/rule-systems`, and `/api/opening-selectors` CRUD, surfaced in Presets as a module library plus Story Director composer.
...
- Presets: Large JSON configs for story directors, event systems, rule systems, and opening selectors now use the Monaco JSON editor with direct editing, scrolling, code folding, default line wrapping, and a single Collapse All / Expand All toggle.
- 方案预设：故事导演内嵌事件系统、数值系统、TRPG 检定、开局选择器，以及独立事件系统、规则系统和开局选择器默认改为可视化编辑；仍可切换 JSON View，视图偏好会保存在浏览器本地，无效 JSON 会阻止保存和切换。
